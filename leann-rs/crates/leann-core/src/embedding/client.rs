...
impl EmbeddingClient {
    ...
    pub fn compute_text_embeddings(&self, chunks: &[String]) -> Result<Array2<f32>> {
        ...
        rt.block_on(async {
            self.compute_text_embeddings_async(chunks).await
        })
    }
    ...
    async fn compute_text_embeddings_async(&self, chunks: &[String]) -> Result<Array2<f32>> {
        ...
        let request = rmp_serde::to_vec(chunks)?;
        let msg = ZmqMessage::from(request);
        socket.send(msg).await.context("sending embedding request")?;

        // Receive response
        let response = socket.recv().await.context("receiving embedding response")?;
        let response_bytes = response.get(0).map(|f| f.as_ref()).unwrap_or(&[]);

        // Decode msgpack response: list of lists of floats
        let embeddings: Vec<Vec<f32>> = rmp_serde::from_slice(response_bytes)
    ...
    pub fn compute_distances(&self, node_ids: &[usize], query: &[f32]) -> Result<Vec<f32>> {
        ...
        rt.block_on(async {
            self.compute_distances_async(node_ids, query).await
        })
    }

    async fn compute_distances_async(
        &self,
        node_ids: &[usize],
        query: &[f32],
    ) -> Result<Vec<f32>> {
        ...
        let request: Vec<serde_json::Value> = vec![
    ...
    pub fn get_embeddings_by_id(&self, node_ids: &[usize]) -> Result<Array2<f32>> {
        ...
        rt.block_on(async {
            self.get_embeddings_by_id_async(node_ids).await
        })
    }
    ...
    async fn get_embeddings_by_id_async(&self, node_ids: &[usize]) -> Result<Array2<f32>> {
        ...
        if flat_data.len() != n * d {
            anyhow::bail!("Data length mismatch: expected {}, got {}", n * d, flat_data.len());
        }
